...
public class TFLiteObjectDetectionAPIModel implements Detector {
  ...
  @Override
  public void setNumThreads(Context context, String modelFilename, int numThreads) throws IOException {
    if (objectDetector != null) {
      optionsBuilder.setNumThreads(numThreads);
      recreateDetector(context, modelFilename);
    }
  }
  ...
  private void recreateDetector(Context context, String modelFilename) throws IOException {
    try {
      if (objectDetector != null) {
        //Closing the Object Detector
        objectDetector.close();
        objectDetector = null;
      }
      modelBuffer = FileUtil.loadMappedFile(context, modelFilename);
